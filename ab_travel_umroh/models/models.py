...
class TravelPackage(models.Model):
    ...
    def compute_quota(self):
        for a in self:
            a.quota_progress = 0
            if a.quota and a.manifest_line:
                a.quota_progress = 100 *  len(a.manifest_line) / a.quota 
                
    
    @api.depends('quota', 'manifest_line')
    def _compute_remaining(self):
        for rec in self:
            rec.remaining_quota = rec.quota - len(rec.manifest_line)

    name = fields.Char('Reference', default='-', readonly=True, store=True, compute='get_name')
    manifest_line = fields.One2many('manifest.line', 'package_manifest_id', string='Manifest Lines', required=True, readonly=True, states={'draft' : [('readonly', False)]})
    hpp_line = fields.One2many('hpp.line', 'package_hpp_id', string='Hpp Lines', required=True, readonly=True, states={'draft' : [('readonly', False)]})
    hotel_line = fields.One2many('hotel.line', 'package_hotel_id', string='Hotel Lines', required=True, readonly=True, states={'draft' : [('readonly', False)]})
    airline_line = fields.One2many('airline.line', 'package_airline_id', string='Airline Lines', required=True, readonly=True, states={'draft' : [('readonly', False)]})
    schedule_line = fields.One2many('schedule.line', 'package_schedule_id', string='Schedule Lines', required=True, readonly=True, states={'draft' : [('readonly', False)]})
    ...
    departure_date = fields.Date('Tanggal Berangkat', required=True, readonly=True, states={'draft' : [('readonly', False)]})
    ...
    remaining_quota = fields.Integer('Remaining Quota', compute='_compute_remaining')
    ...
    ref = fields.Char(string='Kode', default='/', readonly=True)

    @api.depends('ref', 'product_sale_id')
    def get_name(self):
            self.name= str(self.ref) + ' - ' + self.product_sale_id.name
...
class AccountMove(models.Model):
    ...
    def payment_vals(self):
        ...
        payment = str(self.env['account.payment'].search(bill))
        return payment